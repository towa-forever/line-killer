...
export default function SubAccounts({ currentUser, onSwitch, onClose }) {
  ...
  const fetchSubs = useCallback(async () => {
    ...
    try {
      ...
      setSubs(res.data.subs || res.data);
      // サブ垢の場合はAPIレスポンスにmainAccountが含まれる
      if (res.data.mainAccount) {
        setMainAccount(res.data.mainAccount);
      }
    } catch {}
  ...
  const handleSwitch = useCallback(async (targetId) => {
    ...
    try {
      let res;
      if (targetId === currentUser?.parentAccountId) {
        // メインへ戻る
  ...
  }, [currentUser, onSwitch, onClose, showMsg]);
  ...
  const AccountRow = ({ account, isCurrent, isMain }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px',
      background: isCurrent ? 'rgba(6,199,85,0.08)' : 'var(--bg)',
      borderRadius: 14, marginBottom: 8,
      border: isCurrent ? '1.5px solid #06c755' : '1.5px solid transparent' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--text)', display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {isMain && <span style={{ fontSize: 10, background: '#007aff', color: 'white', borderRadius: 8, padding: '1px 6px', fontWeight: 600 }}>メイン</span>}
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
        {isCurrent ? (
          ...
          <>
            <button onClick={() => handleSwitch(account.id)}
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '14px 16px 32px' }}>
          {tab === 'list' && (
            loading ? (
              ...
              <>
                {/* サブ垢の場合：メインアカウントを一覧の先頭に表示 */}
                {isSubAccount && mainAccount && (
                  <>
                    <div style={{ fontSize: 11, color: 'var(--text2)', fontWeight: 600, marginBottom: 8, letterSpacing: 0.3 }}>メインアカウント</div>
                    <AccountRow account={mainAccount} isCurrent={false} isMain={true} />
                    <div style={{ fontSize: 11, color: 'var(--text2)', fontWeight: 600, margin: '12px 0 8px', letterSpacing: 0.3 }}>サブアカウント</div>
                  </>
                )}
                ...
                {!isSubAccount && subs.length > 0 && (
                  <>
                    <div style={{ fontSize: 11, color: 'var(--text2)', fontWeight: 600, margin: '12px 0 8px', letterSpacing: 0.3 }}>サブアカウント ({subs.length}/5)</div>
                    {subs.map(sub => (
                      <AccountRow key={sub.id} account={sub} isCurrent={false} isMain={false} />
                    ))}
                  </>
                )}
          ...
          {tab === 'create' && !isSubAccount && (
            <div>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <input className="form-input" placeholder="半角英数字（3文字以上）" value={form.username}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 20, padding: 24, width: '100%', maxWidth: 320 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ fontSize: 13, color: 'var(--text2)', textAlign: 'center', marginBottom: 20, lineHeight: 1.6 }}>
              「{confirmDelete.displayName || confirmDelete.username}」を完全に削除します。<br />この操作は取り消せません。
            </div>
